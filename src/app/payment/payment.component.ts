...
export class PaymentComponent implements OnInit {
  ...
  constructor(private router: Router, private http: HttpClient, private razorService: RazorService, private zone: NgZone) {
    this._window = this.razorService.nativeWindow;
  }
  ...
  initPay(formTextData: any) {
    ...
    if (formTextData.paymentmode === 'Online') {
      this.razorPayOptions.amount = this.price * 100;
      //comment samaj k delete ni karna
      // @ts-ignore
      this.rzp = new this.razorService.nativeWindow['Razorpay'](this.razorPayOptions);
      // this.rzp = new this.razorparService.nativeWindow.Razorpay(this.razorPayOptions);
      this.rzp.open();
    } else {
      this.http.post(this.url + 'staff/updateOrderPayment', formTextData).subscribe((res1: any) => {
        this.getAllServedOrder();
      });
    }
  }
  ...
  razorPayHandler(res: any) {
    ...
    this.zone.run(() => {
      ...
      this.http.post(this.url + 'staff/updateOrderPayment', this.formTextData).subscribe((res1: any) => {
        this.getAllServedOrder();
      });
    });
  }
  ...
  getAllServedOrder() {
    ...
    this.http.post(this.url + 'staff/getOrderDetails', {msg: 'allserved'}).subscribe((resorder: any) => {
      // if (resorder.msg === 'success') {
        this.allOrders = resorder;
      // }
  ...
  getorderid(orderid: any, amount: any) {
  ...
  searchMobileNo(mobileno) {
    ...
    this.http.post(this.url + 'staff/getOrderDetails', {msg: mobileno}).subscribe((res: any) => {
      this.getPaymentDetails = res;
      // console.log(this.getPaymentDetails[0].amount);
    });
  }
