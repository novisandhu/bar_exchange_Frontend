...
export class ManageCartComponent implements OnInit {
  ...
  // Initialize Razorpay
  initPay(formTextData: any) {
    this.formTextData = formTextData;
    this.razorPayOptions.amount = formTextData.amount * 100;


    //comment samaj k delete ni karna
    // @ts-ignore
  ...
  razorPayHandler(res: any) {
    ...
    this.zone.run(() => {
      ...
      this.http.post(this.url + '/razorpay', this.formTextData).subscribe((res: any) => {
        console.log(res);
      });
    });
  }
  ...
  removeFromCart(productId: any) {
    this.http.post(this.url + 'staff/addtoCart', {pid: productId, action: 'remove'}).subscribe((res: any) => {
  ...
  updateCart(productId: any, act) {

    this.http.post(this.url + 'staff/addtoCart', {pid: productId, action: act}).subscribe((res: any) => {
  ...
  getTotal() {
    ...
    for (let i = 0; i < this.getCart.length; i++) {
      if (this.getCart[i].discount > 0) {
        let showPrice = this.getCart[i].price - (this.getCart[i].price * this.getCart[i].discount / 100);
        this.tab[i] = showPrice;
        this.total[i] = showPrice * this.getCart[i].qty;
      } else {
        this.tab[i] = this.getCart[i].price;
        this.total[i] = this.getCart[i].price * this.getCart[i].qty;
    ...
    for (let i = 0; i < this.total.length; i++) {
      ...
      this.gTotal += parseInt(this.total[i]);
    }

  }

  addOrderDetail(orderData: any) {
    ...
    cartArray.push(objOrderDetail);
    for (let i = 0; i < this.getCart.length; i++) {
      let objProduct = {
        productid: this.getCart[i].productid,
        price: this.getCart[i].price,
        qty: this.getCart[i].qty,
        discount: this.getCart[i].discount,
        netprice: this.getCart[i].price - (this.getCart[i].price * this.getCart[i].discount / 100)
      };
    ...
    this.http.post(this.url + 'staff/addOrder', {msg: cartArray}).subscribe((res: any) => {
